...
class Logger(object):
    ...
    def update_graph(self,model,input_array:None) -> None:
        if self.log_graph : 
            if input_array is None:    
                input_array = torch.rand((1,40,192,192)).float()
    ...
    def update_image(self,image,step: Optional[int],tag:str) -> None: 
        ...
        if image.ndim == 5: # batch 3d image
            self.writer.add_images(f'{tag}/image',image[samplenum],step,dataformats='NCHW') # stacked 3d image
            self.writer.add_video(f'{tag}/image',image[samplenum],step,dataformats='NCHW') # plot 3d video 
    ...
    def confusionmetric(self,label,pred): 
        ...
        # pred,label = self.torch2numpy(pred),self.torch2numpy(label)
        
        # pred,label =list(map(self.check_onehotlist,[pred,label]))

        # print(np.array(pred),np.array(label),np.array(label).shape,np.array(pred).shape)
        ...
        # loc,numlabel = np.unique(label,return_counts=True)
        
        # print(label_num)
        # for i in loc:
        #     label_num[i] = numlabel
